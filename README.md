...
YTSceneSearchは、思い出せそうで思い出せないシーンを自分が覚えてる単語を複数入力すれば該当シーンがでてくる確率がぐーんと上がるプログラム

検索結果には時間指定付きのURLが含まれてます。ワンクリックで該当シーンをブラウザで開きます。わ～便利～w
...
概要欄に元動画を記載されてない場合に使えます。(昔は元動画貼るガイドラインがまだなかったので)


# モデル配布

自分がホロライブの字幕検索のためだけに作ってたので現在はホロ専用です。

2017/09/07～2023/04/18までの全員分の字幕あります。 https://huggingface.co/datasets/keimaru/JP_Holo_Subtitles/tree/main

(更新)チャットバージョンも作りました https://huggingface.co/keimaru/JP_Holo_chat/tree/main
...
でも「昨夜」「昨晩」「昨今」などもヒットするので処理時間増加したり、検索結果が膨大になったりします、工夫して使ってください。
